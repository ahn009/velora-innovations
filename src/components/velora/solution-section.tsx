...
const solutions: Solution[] = [
  { id: 'ai-receptionist', title: 'AI Receptionist', outcome: 'Give every routine inquiry a clear next step.', description: 'Answers approved questions, captures customer details, routes requests, and escalates exceptions to your team.', tags: ['Voice', 'Web', 'SMS'], icon: Headphones, href: '/solutions/ai-receptionist' },
  { id: 'lead-qualification', title: 'Lead Qualification', outcome: 'Send the right opportunities to the right person.', description: 'Collects approved qualification details, identifies intent, and routes each inquiry using clear business rules.', tags: ['Web', 'SMS', 'CRM'], icon: UserRoundCheck, href: '/solutions/lead-qualification' },
  { id: 'appointment-automation', title: 'Appointment Automation', outcome: 'Turn suitable inquiries into confirmed times.', description: 'Checks permitted availability, books eligible appointments, sends confirmations, and keeps exceptions visible.', tags: ['Calendar', 'SMS'], icon: CalendarCheck2, href: '/solutions/appointment-automation' },
...
export function SolutionSection({ variant = 'featured' }: { variant?: 'featured' | 'catalog' }) {
  ...
  return (
    <Section id="solutions">
      <SectionHeading label={variant === 'catalog' ? 'Solution Catalog' : 'Featured Solutions'} title={variant === 'catalog' ? 'Choose the capability that matches the bottleneck' : 'A focused system for the workflow that needs attention first.'} description={variant === 'catalog' ? 'Compare customer communication, revenue operations, and business operations by the job they are designed to handle.' : 'Start with one scoped process, connect the tools that matter, and expand only when the workflow is tested and owned.'} />
