...
export function IndustrySection({ variant = 'featured' }: { variant?: 'featured' | 'directory' }) {
  ...
  return (
    <Section id="industries" background="muted">
      <SectionHeading label={variant === 'directory' ? 'Industry Directory' : 'Featured Industries'} title={variant === 'directory' ? 'Choose your industry' : 'See how the workflow changes by industry.'} description={variant === 'directory' ? 'Compare common operational problems, useful systems, and the responsibilities that should remain with people.' : 'Choose the environment that feels closest to your business to see a practical example of how a scoped workflow could be structured.'} />
        ...
        <div role="tablist" aria-label="Industries" className="grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-2">
          {visibleIndustries.map((industry) => {
            ...
            const stateClasses = isActive
              ? 'border-brand-primary/35 bg-surface-primary text-text-primary shadow-[var(--shadow-soft)]'
              : 'border-border-subtle bg-surface-secondary/60 text-text-secondary hover:border-brand-primary/20 hover:bg-surface-primary'

            return (
              <button
                key={industry.id}
                id={`${panelId}-${industry.id}`}
                type="button"
                role="tab"
                aria-selected={isActive}
                aria-controls={panelId}
                onClick={() => setSelected(industry.id)}
                className={`${variant === 'directory' ? 'grid min-h-20 grid-cols-[34%_1fr] items-center gap-2 p-2 sm:gap-3' : 'flex min-h-16 items-center gap-3 px-3'} rounded-[var(--radius-md)] border text-left text-sm font-semibold transition-[border-color,background-color,box-shadow] duration-[var(--motion-fast)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary/50 ${stateClasses}`}
              >
                {variant === 'directory' ? (
                  <IndustryImage visual={industryVisuals[industry.visualId]} variant="thumbnail" decorative />
                ) : null}
                <span className="flex min-w-0 items-center gap-2">
                  <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-[var(--radius-sm)] ${isActive ? 'bg-brand-primary text-brand-primary-foreground' : 'bg-brand-primary/[0.09] text-brand-primary'}`}>
                    <Icon className="h-4 w-4" aria-hidden="true" />
                  </span>
                  <span className="min-w-0 leading-5">{industry.title}</span>
                </span>
              </button>
            )
          })}
