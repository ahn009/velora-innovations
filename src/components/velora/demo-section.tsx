...
const scenarios: { id: DemoScenario; label: string; customer: string; agent: string; steps: string[]; boundary: string }[] = [
  { id: 'home-services', label: 'Home Services', customer: 'My AC stopped working and I need someone tomorrow.', agent: 'I can collect the service address, check the service area and urgency rules, then offer an eligible next step.', steps: ['Service request', 'Location collected', 'Urgency rule', 'Availability checked', 'Appointment or dispatch', 'CRM + team notification'], boundary: 'No equipment diagnosis or independent safety determination.' },
  { id: 'dental', label: 'Dental', customer: 'I am a new patient and would like to schedule a cleaning.', agent: 'I can collect the administrative details and route a supported appointment request to the practice.', steps: ['New patient', 'Service identified', 'Admin details', 'Appointment type', 'Booking request', 'Front desk handoff'], boundary: 'No diagnosis, treatment recommendation, or clinical intake.' },
  { id: 'law', label: 'Law Firm', customer: 'I need to speak with someone about an immigration matter.', agent: 'I can collect broad administrative intake details and help request a consultation with the firm.', steps: ['Matter category', 'Admin intake', 'Configured review', 'Consultation route', 'Intake record', 'Human attorney or staff'], boundary: 'No legal advice, case strategy, or attorney-client relationship.' },
  { id: 'real-estate', label: 'Real Estate', customer: 'I am thinking about selling my home in the next three months.', agent: 'I can collect the property location, timeline, and contact details before routing the enquiry.', steps: ['Seller intent', 'Property location', 'Timeline', 'Contact details', 'Consultation', 'CRM assignment'], boundary: 'No valuation, financing advice, or negotiation.' },
...
export function DemoSection() {
  ...
  return <Section id="demo" background="muted"><SectionHeading label="Interactive Demo" title="See an AI workflow move from enquiry to action" description="Explore a scripted example of how an agent can understand a business request, apply rules, update systems, and hand off when a person should take over." />
    <div className="mx-auto max-w-6xl rounded-[var(--radius-xl)] border border-border-subtle bg-surface-primary p-3 shadow-card sm:p-5 lg:p-6">
      <div className="mb-5 rounded-[var(--radius-lg)] border border-brand-primary/20 bg-brand-primary/[0.04] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="eyebrow">Choose a business scenario</p><p className="mt-1 text-sm leading-6 text-text-secondary">The interface stays consistent; the operational questions, handoff, and workflow change by industry.</p></div><p className="max-w-sm text-xs leading-5 text-text-muted">Boundary: {scenario.boundary}</p></div><div className="mt-4 flex flex-wrap gap-2" role="tablist" aria-label="Business scenarios">{scenarios.map((item) => <button key={item.id} type="button" role="tab" aria-selected={scenarioId === item.id} onClick={() => { setScenarioId(item.id); setStep(0) }} className={`rounded-full border px-3 py-2 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary ${scenarioId === item.id ? 'border-brand-primary bg-brand-primary text-brand-primary-foreground' : 'border-border-subtle bg-surface-primary text-text-secondary hover:border-brand-primary/30'}`}>{item.label}</button>)}</div><div className="mt-4 grid gap-2 sm:grid-cols-3">{scenario.steps.slice(0, 3).map((item, index) => <div key={item} className="rounded-[var(--radius-md)] bg-surface-primary p-3 text-xs font-medium text-text-secondary"><span className="mr-2 text-brand-hover">0{index + 1}</span>{item}</div>)}</div></div><div className="flex flex-col gap-4 border-b border-border-subtle pb-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-wider text-text-muted">Interactive Demo</p><p className="mt-1 text-sm text-text-secondary">{scenario.label} scripted preview · not a live customer system</p></div><div className="flex flex-wrap gap-2" role="tablist" aria-label="Demo modes">{modes.map(({ id, label, icon: Icon }) => <button key={id} id={`${tabId}-${id}-tab`} type="button" role="tab" aria-selected={mode === id} aria-controls={`${tabId}-${id}-panel`} tabIndex={mode === id ? 0 : -1} onClick={() => { setMode(id); setStep(0) }} onKeyDown={(event) => { const index = modes.findIndex((item) => item.id === mode); const next = event.key === 'ArrowRight' ? (index + 1) % modes.length : event.key === 'ArrowLeft' ? (index - 1 + modes.length) % modes.length : event.key === 'Home' ? 0 : event.key === 'End' ? modes.length - 1 : -1; if (next >= 0) { event.preventDefault(); const nextMode = modes[next].id; setMode(nextMode); setStep(0); document.getElementById(`${tabId}-${nextMode}-tab`)?.focus() } }} className={`inline-flex min-h-10 items-center gap-2 rounded-[var(--radius-md)] px-3 text-xs font-semibold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus-visible:ring-offset-2 ${mode === id ? 'bg-text-primary text-white' : 'bg-background-secondary text-text-secondary hover:bg-border-subtle'}`}><Icon className="h-4 w-4" aria-hidden="true" />{label}</button>)}</div></div>
